 ...
 * @author frankpeeters
 */
public class ClassDiagramPanel extends JPanel implements Dockable {
    ...
    public ClassDiagramPanel(String name, Desktop desktop, boolean newPanel) {
        key = new DockKey(name);
        this.key.setCloseEnabled(false);
        init();

        this.classDiagram = new ClassDiagram(name, desktop.getCurrentProject().getObjectModel());

        postInit(newPanel);
    }

    public ClassDiagramPanel(String name, Desktop desktop, Collection<FactType> selection, boolean newPanel) {
        key = new DockKey(name);
    ...
    public ClassDiagramPanel(Desktop desktop, ClassDiagram classDiagram) {
...
//                        desktop.getPropertyEditor().updateUI();
                ...
                if (e.getButton() == MouseEvent.BUTTON3) {
                    CDPopup cdMenu = new CDPopup(ClassDiagramPanel.this, e.getX(), e.getY());
...
//        key = new DockKey((String) in.readObject());
//        init();
//        initClassBoxes();
//        postInit();
//
//        // For a new diagram we use a layout to set the default locations of the
//        // classboxes.
